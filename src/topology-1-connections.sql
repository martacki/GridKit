...
-- line count(*) = 2
create table topology_edges_helper_2 (
    line_id integer primary key,
	station_id integer array
);

create table topology_edges_2 (
    line_id integer primary key,
    station_id integer array,
    line_extent geometry(linestring, 3857),
    topology_name varchar(64)
...
insert into topology_edges_helper_2 (line_id, station_id)
    select line_id, array_agg(station_id)
	from topology_connections group by line_id having count(*) = 2;

insert into topology_edges_2 (line_id, station_id, line_extent, topology_name)
   select c.line_id, c.station_id, l.extent, l.power_name
   from topology_edges_helper_2 c
    join power_line l on c.line_id = l.line_id;

-- line count(*) > 2
create table topology_edges_helper_3 (
    line_id integer primary key,
	station_id integer array
);

create table topology_edges_3 (
    line_id integer primary key,
    station_id integer array,
    line_extent geometry(linestring, 3857),
    topology_name varchar(64)
...
insert into topology_edges_helper_3 (line_id, station_id)
    select line_id, array_agg(station_id)
	from topology_connections group by line_id having count(*) > 2;

insert into topology_edges_3 (line_id, station_id, line_extent, topology_name)
   select c.line_id, c.station_id, l.extent, l.power_name
   from topology_edges_helper_3 c
    join power_line l on c.line_id = l.line_id;

-- line count(*) > 3
create table topology_edges_helper_4 (
    line_id integer primary key,
	station_id integer array
);

create table topology_edges_4 (
    line_id integer primary key,
    station_id integer array,
    line_extent geometry(linestring, 3857),
    topology_name varchar(64)
...
insert into topology_edges_helper_4 (line_id, station_id)
    select line_id, array_agg(station_id)
	from topology_connections group by line_id having count(*) > 3;

insert into topology_edges_4 (line_id, station_id, line_extent, topology_name)
   select c.line_id, c.station_id, l.extent, l.power_name
   from topology_edges_helper_4 c
    join power_line l on c.line_id = l.line_id;

-- line count(*) > 4
create table topology_edges_helper_5 (
    line_id integer primary key,
	station_id integer array
);

create table topology_edges_5 (
    line_id integer primary key,
    station_id integer array,
    line_extent geometry(linestring, 3857),
    topology_name varchar(64)
...
insert into topology_edges_helper_5 (line_id, station_id)
    select line_id, array_agg(station_id)
	from topology_connections group by line_id having count(*) > 4;

insert into topology_edges_5 (line_id, station_id, line_extent, topology_name)
   select c.line_id, c.station_id, l.extent, l.power_name
   from topology_edges_helper_5 c
    join power_line l on c.line_id = l.line_id;
